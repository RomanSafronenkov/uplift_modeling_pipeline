...
def fact_predict(y_true: np.ndarray, uplift: np.ndarray, treatment: np.ndarray, n_bins=10):
    # запишем результаты (таргет, аплифт и флаг ЦГ)
    df = pd.DataFrame({'target': y_true, 'treatment': treatment, 'uplift': uplift})
    ...
    # посчитаем фактический аплифт в бине (разницу между целевой и контрольной группами по таргету)
    substract = lambda x: x.iloc[1] - x.iloc[0]

    fact_predict = df.groupby('bin', as_index=False)['uplift'].mean()
    fact_predict['target'] = (
        df
        .groupby(['bin', 'treatment'], as_index=False)
        .agg({'target': 'mean'})
...
def check_stat_sig_first_bin(y_true: np.ndarray, uplift: np.ndarray, treatment: np.ndarray, n_bins=10):
    df = pd.DataFrame({'target': y_true, 'treatment': treatment, 'uplift': uplift})
    df = df.sort_values('uplift', ascending=False).reset_index(drop=True)
...
def plot_uplift_curve(y_true: np.ndarray, uplift: np.ndarray, treatment: np.ndarray, ax=None):
...
def evaluate_uplift_with_bootstrap(y_true: np.ndarray, uplift: np.ndarray, treatment: np.ndarray,
    ...
    df_orig = pd.DataFrame({'y': y_true, 'score': uplift, 'w': treatment})
    ...
    if save:
        plt.savefig(f'{save_path}/{save_prefix}qini_ci.png', dpi=300, bbox_inches='tight')
...
def show_results(y_true: np.ndarray, uplift: np.ndarray, treatment: np.ndarray,
                 num_bins: int = 20, save: bool = False, save_path: str = './img', save_prefix: str = '') -> pd.DataFrame:
    """
    ...
    Args:
        ...
        - save_prefix (str): Prefix to add to name of the result image to save.
    ...
    # uplift by percentile plot (fact-predict)
    ...
    # uplift by percentile plot (fact-predict) without predicted uplift
    sns.barplot(pd.melt(f_p, id_vars=['bin'], value_vars=['target']), x='bin', y='value', hue='variable', ax=ax[2])
    ax[1].set_title('Uplift by percentile')
    ...
    if save:
        plt.savefig(f'{save_path}/{save_prefix}uplift_results.png', dpi=300, bbox_inches='tight')
